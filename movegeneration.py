...
def next_move(
    ...
    while time.perf_counter() - t0 < time_limit:
        ...
        if current_move is not None:
            ...
            if debug:
                log_info(f"Depth {depth}: {debug_info['nodes']} nodes searched")
        depth += 1

    debug_info["time"] = time.perf_counter() - t0
    if debug:
        log_info(f"Final stats: {debug_info}")
        log_info(f"Total moves calculated: {debug_info['nodes']}")  # Explicit total
    return best_move if best_move else random.choice(list(board.legal_moves))

# ... [keep order_moves() function identical] ...
